...
export function buildPersonaDraftSystemPrompt(persona: Persona, profile: string): string {
  const profileTruncated = truncateToWords(profile || "No profile provided.", 100);
  const base = `Candidate Profile:\n${profileTruncated}`;

  if (persona === "cold") {
    return `You are helping the candidate write a cold LinkedIn connection request note. Rules:
- Write from the candidate's perspective — they are reaching out
...
${base}`;
  ...
  if (persona === "warm") {
    return `You are helping the candidate write a LinkedIn message to someone they have had prior contact with. Rules:
- Write from the candidate's perspective — they are reaching out, not a recruiter or third party
...
- Line 2: One clear reason the candidate is reaching out now — tie it to their work, company news, or the role
...
${base}`;
  ...
  // hot
  return `You are helping the candidate write a message to a real connection — former colleague, mentor, or someone they know well. Rules:
- Write from the candidate's perspective — lead with the relationship naturally
- Be direct and honest about what they are exploring
- Keep it short — the recipient knows them, no need for a pitch
